...
def compute_degree_cm_mAP(final_results, synset_names, log_dir, degree_thresholds=[360], shift_thresholds=[100], iou_3d_thresholds=[0.1], iou_pose_thres=0.1, use_matches_for_pose=False):
    ...
    for i in range(len(pose_aps)-1):
        ...
        print('5 degree, 10cm: {:.1f}'.format(pose_aps[i, degree_thres_list.index(5),shift_thres_list.index(10)] * 100))
        print('10 degree, 2cm: {:.1f}'.format(pose_aps[i, degree_thres_list.index(10),shift_thres_list.index(2)] * 100))
        print('10 degree, 5cm: {:.1f}'.format(pose_aps[i, degree_thres_list.index(10),shift_thres_list.index(5)] * 100))
        print('10 degree, 10cm: {:.1f}'.format(pose_aps[i, degree_thres_list.index(10),shift_thres_list.index(10)] * 100))
        print('15 degree, 5cm: {:.1f}'.format(pose_aps[i, degree_thres_list.index(15),shift_thres_list.index(5)] * 100))
        print('15 degree, 10cm: {:.1f}'.format(pose_aps[i, degree_thres_list.index(15),shift_thres_list.index(10)] * 100))
    print('mean')
    print('3D IoU at 25: {:.1f}'.format(iou_aps[-1, iou_thres_list.index(0.25)] * 100))
    print('3D IoU at 50: {:.1f}'.format(iou_aps[-1, iou_thres_list.index(0.5)] * 100))
...
def prepare_data_posefitting(sem_mask,coord) :
    ...
    if len(class_ids) == 0:
        return np.zeros((0)) , np.ones((0), dtype=np.float32) ,np.zeros((h,w,0)) ,np.zeros((h,w,0,3)) , np.zeros((0,4))
    if class_ids[0]==0 :
...
def align(class_ids, masks, coords, depth, intrinsics, synset_names, image_path='None', save_path=None, if_norm=False, with_scale=True, verbose=False):
