...
class SwinDRNetDataset(Dataset):
    ...
    def _create_lists_filenames(self, rgb_dir, sim_depth_dir, output_depth_dir,syn_depth_dir, nocs_dir, mask_dir, meta_dir):
        ...
        assert os.path.isdir(output_depth_dir), 'Dataloader given output depth images directory that does not exist: "%s"' % (sim_depth_dir)
        for ext in self._extension_sim_depth:
        ...
        if num_output_depth == 0:
            raise ValueError('No output depth images found in given directory. Searched in dir: {} '.format(sim_depth_search_str))
        if num_output_depth != num_rgb:
            raise ValueError('The number of output depth images and rgb images do not match. Please check data,' +
